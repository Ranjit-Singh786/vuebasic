    ...
    <DashboardLayout>
      ...
      <div  class="dashbpoard-content px-0 px-md-5 py-4">
        <!-- *******team-heading******* -->
          ...
          <div class="user-head ">
          <h2 class="m-0">Position Type :</h2>
        </div>
        <div class="total-department">
          <h5 class="mb-0">Total Position = {{recordData}}</h5>
        </div>
        </div>
        <!-- *******team-content******* -->
        <div class="row">
              ...
              <div class="col-12 col-md-4">
                  <div class="input-group department-input mb-3">
                    ...
                    <input
                      ...
                      placeholder="Search"
                      v-model="posname_filter"
                      @keyup="processChange(get_postion)"
                  ...
                  <div class="filter-icon mb-3">
                    <div class="col-12 col-md-3">
                      <div class="">
                        ...
                        height="24" style="fill: var(--navy-blue)" viewBox="0 0 320 512"><path d="M310.6 361.4c12.5 12.5 12.5 32.75 0 45.25C304.4 412.9 296.2 416 288 416s-16.38-3.125-22.62-9.375L160 301.3L54.63 406.6C48.38 412.9 40.19 416 32 416S15.63 412.9 9.375 406.6c-12.5-12.5-12.5-32.75 0-45.25l105.4-105.4L9.375 150.6c-12.5-12.5-12.5-32.75 0-45.25s32.75-12.5 45.25 0L160 210.8l105.4-105.4c12.5-12.5 32.75-12.5 45.25 0s12.5 32.75 0 45.25l-105.4 105.4L310.6 361.4z"/></svg>
                        </button>
                      </div>
                            ...
                            <div class="modal-body modal-form">
                              <form @submit.prevent="apply_filter_g">
                                <div class="mb-3 modal-input">
                                  ...
                                  <select class="form-select shadow-none" v-model="posname_filter" aria-label="Default select example">
                  ...
                  >
                    <div class="modal-dialog modal-dialog-centered">
                      <div class="modal-content">
                        ...
                        <div class="modal-body modal-form">
                          <form @submit.prevent="add_postion">
                            <div class="mb-3 modal-input">
                              ...
                              <input
                                ...
                                class="form-control"
                                id="recipient-name"
                                v-model="add_pos.postion"
                                placeholder="Position name is required"
                              />
                               <span class="text-danger" v-if="v$.add_pos.postion.$error">{{
                                v$.add_pos.postion.$errors[0].$message
                              }}</span>
                            </div>
                    ...
                    <div class="table-responsive department-table">
                      <table class="table align-middle">
                        ...
                        <tbody> 
                          ...
                          <tr class="text-center department-action" v-for="(postion,index) in postion_data" :key="index">
                            <td>{{perPageData *(page-1)+ index+1}}</td>
                            <td>{{postion.position_name}}</td>
                            <td class="text-start">
                                  ...
                                  >
                                    ...
                                    >
                                      <div class="modal-content">
                                        ...
                                        <div class="modal-body modal-form">
                                          <form @submit.prevent="update_postion(update_r.id)">
                                            <div class="mb-3 modal-input">
                                              <label
                                                for="recipient-name"
                                              ...
                                              <input
                                                ...
                                                class="form-control"
                                                id="recipient-name"
                                                v-model="update_r.postion"
                                              />
                                        <span class="text-danger" v-if="v$.update_r.postion.$error">{{
                                          v$.update_r.postion.$errors[0].$message
                                        }}</span>
                                            </div>
                              ...
                              <button v-if="loadingupdate == false" type="submit" class="btn btn_save border w-100 py-2 shadow-none">
                                ...
                                <div class="edit-icon">
                                  <svg
                                    ...
                                    viewBox="0 0 24 24"
                                    style="fill: var(--red)"
                                    @click="delete_postion(postion.id)"
                                  >
                                    <path
                ...
                <div class="">
                  <div class="show-entry d-flex gap-2 align-items-center">
                    ...
                    <div class="select-entry">
                      <select class="form-select" v-model="perPageData" @change="get_postion">
...
export default {
   ...
   setup() {
      ...
      validations(){
         return {
          add_pos:{
             postion: {
          required: helpers.withMessage("Position name is required", required),
        }
          },
          update_r:{
              postion:{
  ...
  mounted(){
    this.processChange = this.debounce(() => this.get_postion());
    ...
    computed: {
    // filter_data() {
    //   console.log("role data", this.postion_data);

    //   if (this.search) {
    //     return this.postion_data.filter((p) => {
    //       // console.log(p);
  ...
  methods:{
    ...
    update_select(exp){
           console.log(exp);
           this.update_r.id = exp.id;
           this.update_r.postion = exp.position_name;
           
    },
    //update data 
   async update_postion(id){
      ...
      if (!result) {
        return;
             ...
             this.loadingupdate = true;
              let data={
                position_name:this.update_r.postion,
                

              }
               console.log(data);
                let response = await ApiClass.updateNodeRequest("position-update?id="+id , true, data);
    ...
    reset_validaton(){
      ...
      async get_postion() {
        this.loading = true;
      let response = await ApiClass.getNodeRequest("position-get?page="+this.page+"&limit="+this.perPageData+"&postion_name="+this.posname_filter, true);
      ...
      if (response?.data) {
        ...
        if (response?.data?.status_code == "1") {
          console.log(response.data);
            ...
            this.show = true;
            this.postion_data = response?.data?.data?.result ?? [];
    ...
    async add_postion(){
       ...
       const result = await this.v$.add_pos.$validate();
      if (!result) {
        return;
      }
      ...
      let form_data = {
        ...
        position_name:this.add_pos.postion,
       
      };
       console.log(form_data)
      let response = await ApiClass.postNodeRequest("position-create", true, form_data);
      console.log(response.data);
      this.loading = false;
    ...
    //delete api 
     delete_postion(id) {
        // alert(id)
          ...
          async delete_pos(id){
            // alert(id)
          let response = await ApiClass.deleteNodeRequest("position-delete?id="+id , true);
