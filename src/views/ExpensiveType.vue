              ...
              <div class="col-12 col-md-4">
                  <div class="input-group department-input mb-3">
                    ...
                    <input
                      ...
                      @keyup="processChange(get_expenses)"
                            ...
                            <div class="modal-body modal-form">
                              <form @submit.prevent="apply_filter_g">
                                <div class="mb-3 modal-input">
                                  <label for="recipient-name" class="col-form-label">Expensive :</label>
                  ...
                  >
                    <div class="modal-dialog modal-dialog-centered">
                      <div class="modal-content">
                        ...
                        <div class="modal-body modal-form">
                          <form @submit.prevent="add_expeneses">
                            <div class="mb-3 modal-input">
                              ...
                              <input
                                ...
                                class="form-control"
                                id="recipient-name"
                                v-model="add_expen.expen"
                                placeholder="Expenses name is required"
                              />
                               <span class="text-danger" v-if="v$.add_expen.expen.$error">{{
                                v$.add_expen.expen.$errors[0].$message
                              }}</span>
                            </div>
                  ...
                  <div class="col-md-12">
                    <div class="table-responsive department-table">
                      <table class="table align-middle">
                        ...
                        <tbody> 
                          <tr v-if="expenses_data.length ==0">
                                    <td colspan="10">
                          ...
                          <tr class="text-center department-action" v-for="(expen,index) in expenses_data" :key="index">
                            <td>{{perPageData *(page-1)+ index+1}}</td>
                            <td>{{expen.expenses_type}}</td>
                            <td class="text-start">
                                  ...
                                  >
                                    ...
                                    >
                                      <div class="modal-content">
                                        ...
                                        <div class="modal-body modal-form">
                                          <form @submit.prevent="update_expen(update_r.id)">
                                            <div class="mb-3 modal-input">
                                              <label
                                                for="recipient-name"
                                              ...
                                              <input
                                                ...
                                                class="form-control"
                                                id="recipient-name"
                                                v-model="update_r.expen"
                                              />
                                        <span class="text-danger" v-if="v$.update_r.expen.$error">{{
                                          v$.update_r.expen.$errors[0].$message
                                        }}</span>
                                            </div>
                ...
                <div class="">
                  <div class="show-entry d-flex gap-2 align-items-center">
                    ...
                    <div class="select-entry">
                      <select class="form-select" v-model="perPageData" @change="get_expenses">
...
export default {
   ...
   setup() {
      ...
      validations(){
         return {
          add_expen:{
             expen: {
          required: helpers.withMessage("Expensive name is required", required),
        }
          },
          update_r:{
              expen:{
  ...
  mounted(){
    this.processChange = this.debounce(() => this.get_expenses());
    ...
    computed: {
    // filter_data() {
    //   console.log("role data", this.expenses_data);

    //   if (this.search) {
    //     return this.expenses_data.filter((p) => {
    //       // console.log(p);
  ...
  methods:{
    ...
    update_select(exp){
           ...
           this.update_r.expen = exp.expenses_type;
           
    },
    //update data 
   async update_expen(id){
      ...
      if (!result) {
        return;
             ...
             this.loadingupdate = true;
              let data={
                expenses_type:this.update_r.expen,
                

              }
               console.log(data);
                let response = await ApiClass.updateNodeRequest("expenses-type-update?id="+id , true, data);
    ...
    reset_validaton(){
      ...
      async get_expenses() {
        this.loading = true;
      let response = await ApiClass.getNodeRequest("expenses-type-get?page="+this.page+"&limit="+this.perPageData+"&expenses_type="+this.role_name_filter, true);
      ...
      if (response?.data) {
        ...
        if (response?.data?.status_code == "1") {
          console.log(response.data);
            ...
            this.show = true;
            this.expenses_data = response?.data?.data?.result ?? [];
    ...
    async add_expeneses(){
       ...
       const result = await this.v$.add_expen.$validate();
      if (!result) {
        return;
      }
      ...
      let form_data = {
        ...
        expenses_type:this.add_expen.expen,
       
      };
       console.log(form_data)
      let response = await ApiClass.postNodeRequest("expenses-type-create", true, form_data);
      console.log(response.data);
      this.loading = false;
        ...
        // alert(id)
            ...
            this.$swal({
                ...
                if (result.isConfirmed) {
                    this.delete_expensive(id);
                } else {
                    return;
                }
            });
        },
          async delete_expensive(id){
            // alert(id)
          let response = await ApiClass.deleteNodeRequest("expenses-type-delete?id="+id , true);
